<template>
  <div :class="[
    ...
    <div class="p-4 flex flex-col justify-between flex-grow bg-white gap-4">
      <div>
        <h2 id="title" class="line-clamp-2 leading-6 font-bold mb-4">{{ article.title }}</h2>
        <p id="description" class="line-clamp-3 leading-6">{{ article.description }}</p>
      </div>
      <div class="flex items-center justify-between overflow-hidden bg-gray-100 py-2 px-4 gap-5 rounded-lg">
        <div>
          <p id="author" class="max-w-80 overflow-hidden text-ellipsis font-bold line-clamp-1">
            {{ article.author || article.source.name }}
          </p>
          <p id="date" class="text-gray-500 line-clamp-1">{{ formattedDate }}</p>
        </div>
